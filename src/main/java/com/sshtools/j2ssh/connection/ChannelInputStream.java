...
//    }
    ...
    private synchronized void collectNextMessage()
	    throws MessageStoreEOFException, InterruptedException, IOException {
	...
	while ((msg == null)) {
	    try {
		msg = messageStore.getMessage(filter, interrupt);
	    } catch (MessageNotAvailableException ex) {
		// Ignore the timeout but this allows us to review the
		// InputStreams state once in a while

		// if we are closed and there are no more messages waiting
		// break here
		if (isClosed()) {
		    break;
		}
	...
	if (msg != null) {
	    if (type != null) {
		msgdata = ((SshMsgChannelExtendedData) msg).getChannelData();
	    } else {
		msgdata = ((SshMsgChannelData) msg).getChannelData();
	    }
	    currentPos = 0;
	} else {
	    throw new MessageStoreEOFException();
