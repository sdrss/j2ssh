...
public abstract class BindingChannel extends Channel {
    ...
    /**  */
    protected List<SshMessage> messages = new ArrayList<SshMessage>();
    ...
    private void sendOutstandingMessages() throws IOException {
        ...
        synchronized (messages) {
            ...
            for(final SshMessage msg : messages) {

                if (msg instanceof SshMsgChannelData) {
                    boundChannel.sendChannelData(((SshMsgChannelData) msg).getChannelData());
                } else if (msg instanceof SshMsgChannelExtendedData) {
                    boundChannel.sendChannelExtData(((SshMsgChannelExtendedData) msg).getDataTypeCode(),
                        ((SshMsgChannelExtendedData) msg).getChannelData());
                } else {
                    throw new IOException("[" + getName() +
