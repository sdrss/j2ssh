...
public class FileAttributes {
    ...
    Map<String, String> extended = new HashMap<String, String>(); // Version 3 &
								  // 4
    ...
    /*
     * public FileAttributes(int type, int version) { if(type >= 1 && type <= 5)
     * { this.type = type; this.version = version; } else throw new
     * IllegalArgumentException("The type must be a valid FileAttribute type");
     * }
     */
	...
	/*
	 * if(isFlagSet(SSH_FILEXFER_ATTR_OWNERGROUP)) { owner =
	 * bar.readString(); group = bar.readString(); }
	 */
	...
	/*
	 * if(isFlagSet(SSH_FILEXFER_ATTR_CREATETIME)) createtime =
	 * bar.readUINT32(); if(isFlagSet(SSH_FILEXFER_ATTR_MODIFYTIME)) mtime =
	 * bar.readUINT32();
	 */
	/*
	 * if(isFlagSet(SSH_FILEXFER_ATTR_ACL)) { int count =
	 * (int)bar.readInt(); for(int i=0;i<count;i++) { acl.add(new ACL(bar));
	 * } }
	 */
    ...
    /*
     * public void setOwner(String owner) { this.owner = owner; // Set the flag
     * if(group!=null || owner!=null) flags |= SSH_FILEXFER_ATTR_OWNERGROUP;
     * else flags ^= SSH_FILEXFER_ATTR_OWNERGROUP; } public String getOwner() {
     * return owner; }
     */
    /*
     * public void setGroup(String group) { this.group = group; // Set the flag
     * if(group!=null || owner!=null) flags |= SSH_FILEXFER_ATTR_OWNERGROUP;
     * else flags ^= SSH_FILEXFER_ATTR_OWNERGROUP; } public String getGroup() {
     * return group; }
     */
	...
	if (permissions != null) {
	    cp = cp
		    | (((permissions.intValue() & S_IFMT) == S_IFMT) ? S_IFMT
			    : 0);
	    cp = cp
		    | (((permissions.intValue() & S_IFSOCK) == S_IFSOCK) ? S_IFSOCK
			    : 0);
	    cp = cp
		    | (((permissions.intValue() & S_IFLNK) == S_IFLNK) ? S_IFLNK
			    : 0);
	    cp = cp
		    | (((permissions.intValue() & S_IFREG) == S_IFREG) ? S_IFREG
			    : 0);
	    cp = cp
		    | (((permissions.intValue() & S_IFBLK) == S_IFBLK) ? S_IFBLK
			    : 0);
	    cp = cp
		    | (((permissions.intValue() & S_IFDIR) == S_IFDIR) ? S_IFDIR
			    : 0);
	    cp = cp
		    | (((permissions.intValue() & S_IFCHR) == S_IFCHR) ? S_IFCHR
			    : 0);
	    cp = cp
		    | (((permissions.intValue() & S_IFIFO) == S_IFIFO) ? S_IFIFO
			    : 0);
	    cp = cp
		    | (((permissions.intValue() & S_ISUID) == S_ISUID) ? S_ISUID
			    : 0);
	    cp = cp
		    | (((permissions.intValue() & S_ISGID) == S_ISGID) ? S_ISGID
			    : 0);
	}
	...
	if (len >= 1) {
	    cp = cp
		    | ((newPermissions.charAt(0) == 'r') ? FileAttributes.S_IRUSR
			    : 0);
	}

	if (len >= 2) {
	    cp = cp
		    | ((newPermissions.charAt(1) == 'w') ? FileAttributes.S_IWUSR
			    : 0);
	}

	if (len >= 3) {
	    cp = cp
		    | ((newPermissions.charAt(2) == 'x') ? FileAttributes.S_IXUSR
			    : 0);
	}

	if (len >= 4) {
	    cp = cp
		    | ((newPermissions.charAt(3) == 'r') ? FileAttributes.S_IRGRP
			    : 0);
	}

	if (len >= 5) {
	    cp = cp
		    | ((newPermissions.charAt(4) == 'w') ? FileAttributes.S_IWGRP
			    : 0);
	}

	if (len >= 6) {
	    cp = cp
		    | ((newPermissions.charAt(5) == 'x') ? FileAttributes.S_IXGRP
			    : 0);
	}

	if (len >= 7) {
	    cp = cp
		    | ((newPermissions.charAt(6) == 'r') ? FileAttributes.S_IROTH
			    : 0);
	}

	if (len >= 8) {
	    cp = cp
		    | ((newPermissions.charAt(7) == 'w') ? FileAttributes.S_IWOTH
			    : 0);
	}

	if (len >= 9) {
	    cp = cp
		    | ((newPermissions.charAt(8) == 'x') ? FileAttributes.S_IXOTH
			    : 0);
	}
    ...
    /*
     * public void setCreatedTime(UnsignedInteger32 atime) { this.createtime =
     * createtime; // Set the flag if(createtime!=null) flags |=
     * SSH_FILEXFER_ATTR_CREATETIME; else flags ^= SSH_FILEXFER_ATTR_CREATETIME;
     * } public UnsignedInteger32 getCreatedTime() { return createtime; } public
     * void setModifiedTime(UnsignedInteger32 atime) { this.mtime = mtime; //
     * Set the flag if(mtime!=null) flags |= SSH_FILEXFER_ATTR_MODIFYTIME; else
     * flags ^= SSH_FILEXFER_ATTR_MODIFYTIME; } public UnsignedInteger32
     * getModifiedTime() { return mtime; }
     */
    /*
     * public List getACL() { return acl; }
     */
	...
	if (extended.size() > 0) {
	    ...
	    /*
	     * if(acl.size() > 0) flags |= SSH_FILEXFER_ATTR_ACL;
	     */
	}
	...
	/*
	 * if(isFlagSet(SSH_FILEXFER_ATTR_OWNERGROUP)) { baw.writeString(owner);
	 ...
	 * if(isFlagSet(SSH_FILEXFER_ATTR_MODIFYTIME)) baw.writeUINT32(mtime);
	 * if(isFlagSet(SSH_FILEXFER_ATTR_ACL)) { ByteArrayWriter acls = new
	 * ByteArrayWriter(); acls.writeInt(acl.size()); Iterator it =
	 * acl.iterator(); while(it.hasNext()) {
	 * acls.write(((ACL)it.next()).toByteArray()); }
	 * baw.writeBinaryString(acls.toByteArray()); acls = null; }
	 */
	if (isFlagSet(SSH_FILEXFER_ATTR_EXTENDED)) {
	    baw.writeInt(extended.size());

	    Iterator<Map.Entry<String, String>> it = extended.entrySet()
		    .iterator();
	    while (it.hasNext()) {
		Map.Entry<String, String> entry = it.next();
		baw.writeString(entry.getKey());
		baw.writeString(entry.getValue());
	    }
	}
