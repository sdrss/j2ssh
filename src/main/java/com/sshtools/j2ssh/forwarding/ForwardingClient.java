...
//    }
    ...
    public void synchronizeConfiguration(SshConnectionProperties properties) {
        ...
        if (properties.getLocalForwardings().size() > 0) {
            for (Iterator<ForwardingConfiguration> it = properties.getLocalForwardings().values()
                                         .iterator(); it.hasNext();) {
                try {
        ...
        if (properties.getRemoteForwardings().size() > 0) {
            for (Iterator<ForwardingConfiguration> it = properties.getRemoteForwardings().values()
                                         .iterator(); it.hasNext();) {
                try {
                    fwd = (ForwardingConfiguration) it.next();
