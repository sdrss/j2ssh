...
public class SshClient {
    ...
    protected ArrayList<Channel> activeChannels = new ArrayList<Channel>();

    /**
     * An channel event listener implementation to maintain the active channel
     * list.
     */
    ...
    private ArrayList<SftpClient> activeSftpClients = new ArrayList<SftpClient>();

    /**
     * <p>
     * Constructs an uninitialized SshClient ready for connecting.
     * </p>
     */
    public SshClient() {
    }
     ...
     * @since 0.2.0
     */
    public List<String> getAvailableAuthMethods(String username)
        throws IOException {
        if (authentication != null) {
    ...
    public List<Channel> getActiveChannels() {
        synchronized (activeChannels) {
            return new ArrayList<Channel>(activeChannels);
//            return (List<Channel>)activeChannels.clone();
    ...
    public boolean hasActiveSession(String type) {
        for(final Channel channel : activeChannels) {
            if (channel instanceof SessionChannelClient) {
                if (((SessionChannelClient) channel).getSessionType().equals(type)) {
                    return true;
                }
            }
        }
    ...
    public SessionChannelClient getActiveSession(String type)
        ...
        for(final Channel channel : activeChannels) {
            if (channel instanceof SessionChannelClient) {
                if (((SessionChannelClient) channel).getSessionType().equals(type)) {
                    return (SessionChannelClient) channel;
                }
            }
        }
