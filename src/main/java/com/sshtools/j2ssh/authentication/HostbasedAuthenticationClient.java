...
public class HostbasedAuthenticationClient extends SshAuthenticationClient {
    private static Log log = LogFactory
	    .getLog(HostbasedAuthenticationClient.class);
     ...
     * SshPrivateKeyFile pkf = null; if (privateKeyFile == null) { JFileChooser
     * chooser = new JFileChooser(); chooser.setFileHidingEnabled(false);
     * chooser.setDialogTitle("Select Private Key File For Authentication"); if
     * (chooser.showOpenDialog(parent) == JFileChooser.APPROVE_OPTION) {
     * privateKeyFile = chooser.getSelectedFile().getAbsolutePath(); } else {
     * return false; } } FileInputStream in = null; try { pkf =
     * SshPrivateKeyFile.parse(new File(privateKeyFile)); } catch
     * (InvalidSshKeyException iske) { JOptionPane.showMessageDialog(parent,
     * iske.getMessage()); return false; } catch (IOException ioe) {
     * JOptionPane.showMessageDialog(parent, ioe.getMessage()); } // Now see if
     * its passphrase protected if (pkf.isPassphraseProtected()) { if
     * (passphrase == null) { // Show the passphrase dialog Window w = (Window)
     * SwingUtilities.getAncestorOfClass(Window.class, parent); PassphraseDialog
     * dialog = null; if (w instanceof Frame) { dialog = new PassphraseDialog(
     * (Frame) w); } else if (w instanceof Dialog) { dialog = new
     * PassphraseDialog( (Dialog) w); } else { dialog = new PassphraseDialog();
     * } do { dialog.setVisible(true); if (dialog.isCancelled()) { return false;
     * } passphrase = new String(dialog.getPassphrase()); try { key =
     * pkf.toPrivateKey(passphrase); break; } catch (InvalidSshKeyException
     * ihke) { dialog.setMessage("Passphrase Invalid! Try again");
     * dialog.setMessageForeground(Color.red); } } while (true); } else { try {
     * key = pkf.toPrivateKey(passphrase); } catch (InvalidSshKeyException ihke)
     * { return false; } } } else { try { key = pkf.toPrivateKey(null); } catch
     * (InvalidSshKeyException ihke) { JOptionPane.showMessageDialog(parent,
     * ihke.getMessage()); return false; } } return true; }
     */
