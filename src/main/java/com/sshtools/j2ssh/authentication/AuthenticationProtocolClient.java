...
public class AuthenticationProtocolClient extends Service {
    private static Log log = LogFactory
	    .getLog(AuthenticationProtocolClient.class);
    private int[] resultFilter = new int[2];
    private int[] singleIdFilter = new int[3];
    private List<AuthenticationProtocolListener> listeners = new ArrayList<AuthenticationProtocolListener>();
	...
	messageStore.registerMessage(
		SshMsgUserAuthBanner.SSH_MSG_USERAUTH_BANNER,
     ...
     * @throws SshException
     */
    public List<String> getAvailableAuths(String username, String serviceName)
	    throws IOException {
	...
	if (msg instanceof SshMsgUserAuthFailure) {
		...
		throw new TerminatedStateException(
	...
	try {
	    ...
	    SshMessage msg = messageStore.peekMessage(
		    SshMsgUserAuthBanner.SSH_MSG_USERAUTH_BANNER, timeout);
