 ...
 * sshtools agent; the agent stores private keys and can hash and sign data for
 * the public key authentication request.
 * </p>
 *
...
public class AgentAuthenticationClient extends SshAuthenticationClient {
    private static Log log = LogFactory
	    .getLog(PublicKeyAuthenticationClient.class);
    ...
    /*
     * public void setKey(SshPublicKey key) { this.key = key; }
     */
	...
	try {
	    msg = authentication
		    .readMessage(SshMsgUserAuthPKOK.SSH_MSG_USERAUTH_PK_OK);
	...
	Iterator<Map.Entry<SshPublicKey, String>> it = keys.entrySet()
		.iterator();
	...
	while (it.hasNext() && !acceptable) {
		...
		SshMsgUserAuthRequest msg = new SshMsgUserAuthRequest(
			getUsername(), serviceToStart, getMethodName(),
			baw.toByteArray());
		authentication.sendMessage(msg);
	...
	try {
	    ...
	    for (Iterator<SshPublicKey> x = keys.keySet().iterator(); x
		    .hasNext();) {
		key = x.next();
