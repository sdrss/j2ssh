...
public abstract class AbstractHostKeyVerification extends DefaultHandler
	...
	try {
		...
		// Load the hosts file. Do not worry if fle doesnt exist, just
		// disable
		// save of
     ...
     * @return
     */
    public Map<String, String> allowedHosts() {
	return allowedHosts;
    }
     ...
     * @return
     */
    public java.util.List<String> deniedHosts() {
	return deniedHosts;
    }
	...
	Iterator<Map.Entry<String, String>> it = allowedHosts.entrySet()
		.iterator();

	while (it.hasNext()) {
	    Map.Entry<String, String> entry = it.next();
	    xml += ("   " + "<AllowHost HostName=\"" + entry.getKey()
		    + "\" Fingerprint=\"" + entry.getValue() + "\"/>\n");
	}

	xml += "   <!-- Deny the following hosts access -->\n";

	for (final String host : deniedHosts) {
	    xml += ("   <DenyHost HostName=\"" + host + "\"/>\n");
	}
