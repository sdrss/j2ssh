...
public class SSH2AuthorizedKeysFormat implements AuthorizedKeysFormat {
	...
	for (Iterator<Map.Entry<SshPublicKey, String>> it = keys
		.getAuthorizedKeys().entrySet().iterator(); (it != null)
		&& it.hasNext();) {
	    Map.Entry<SshPublicKey, String> entry = it.next();
    ...
    public AuthorizedKeys unformat(byte[] formatted,
	    AuthorizedKeysFileLoader loader) throws IOException,
	    InvalidSshKeyException {
