...
public class RemoteIdentificationFactory {
    private static Map<String, RemoteIdentification> remoteIdentifications = null;

    static {
	try {
	    if (ConfigurationLoader
		    .isConfigurationAvailable(AutomationConfiguration.class)) {
    ...
    public static synchronized RemoteIdentification getInstance(
	    String serverId, String hostname)
	    throws RemoteIdentificationException {
	...
	Iterator<Map.Entry<String, RemoteIdentification>> it = remoteIdentifications
		.entrySet().iterator();
	Map.Entry<String, RemoteIdentification> entry;
