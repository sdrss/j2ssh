...
public class RemoteIdentification {
    ...
    private Vector<RemoteIdentificationRule> rules = new Vector<RemoteIdentificationRule>();
     ...
     * @return
     */
    protected List<RemoteIdentificationRule> getRules() {
	return rules;
    }
	...
	// boolean pass = false;
	Iterator<RemoteIdentificationRule> it = rules.iterator();
	...
	try {
		...
		return (AuthorizedKeysFormat) authorizedKeysFormat
			.newInstance();
	    } else {
		throw new RemoteIdentificationException(
    ...
    public boolean configureUserAccess(final SftpClient sftp,
	    final String serverId, String system, String username, List<SshPublicKey> keys,
	    String authorizationFile, int mode)
	    throws RemoteIdentificationException {
	try {
	    if (sftp.isClosed()) {
	    ...
	    final String directory = ((authorizationFile.lastIndexOf("/") > 0) ? authorizationFile
		    .substring(0, authorizationFile.lastIndexOf("/") + 1) : "");
	    ...
	    out.write(AuthorizedKeys.create(authorizedKeys, serverId, system,
			...
			public void saveFile(String filename, byte[] filedata)
				...
				log.info(
					"Error writing public key file to server"
						+ filename, ex);
			    } finally {
				if (in != null) {
				    in.close();
