...
public class XmlConfigurationContext implements ConfigurationContext {
	...
	if (apiResource != null) {
	    try {
		SshAPIConfiguration x = new SshAPIConfiguration(
			ConfigurationLoader.loadFile(apiResource));
		configurations
			.put(com.sshtools.j2ssh.configuration.SshAPIConfiguration.class,
				x);
	    } catch (Exception ex) {
		if (failOnError) {
	...
	if (automationResource != null) {
		...
		AutomationConfiguration y = new AutomationConfiguration(
			ConfigurationLoader.loadFile(automationResource));
		configurations
			.put(com.sshtools.common.automate.AutomationConfiguration.class,
				y);
	    } catch (Exception ex) {
		if (failOnError) {
		    throw new ConfigurationException(ex.getMessage());
