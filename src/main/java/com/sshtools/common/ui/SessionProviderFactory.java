 ...
 * 
 * <p>
 * This class is responsible for dynamically loading all the installed session
 * providers. A session provider can be used with
 * <code>SessionProviderFrame</code> to integrate an ssh service such as a
 * terminal window or sftp window within another application.
 * </p>
 * 
 * <p>
 * To install a session provider you should provide a special properties file
 * resource at the root of your source tree called 'session.provider'.
 * </p>
 * 
 * <blockquote>
 * 
 ...
 * @version $Id: SessionProviderFactory.java,v 1.12 2003/09/22 15:57:57 martianx
 *          Exp $
 */
public class SessionProviderFactory {
    ...
    HashMap<String, SessionProvider> providers = new HashMap<String, SessionProvider>();
	...
	try {
	    Enumeration<URL> en = classloader.getResources("session.provider");
	    URL url = null;
		...
		try {
			...
			Class<?> cls = classloader.loadClass(properties
				.getProperty("provider.class"));
			String optionsClassName = properties
				.getProperty("provider.options");
			Class<?> optionsClass = ((optionsClassName == null) || optionsClassName
				.equals("")) ? null : classloader
				.loadClass(optionsClassName);
			String pageclass;
			int num = 1;
			Vector<Class<?>> pages = new Vector<Class<?>>();
			...
			int weight = Integer.parseInt(properties
				.getProperty("provider.weight"));
	...
	return new Vector<SessionProvider>(providers.values());
    }

    /**
     * <p>
     * Get a <code>SessionProvider</code> by its id. The id is defined by the
