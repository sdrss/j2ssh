 ...
 */
public abstract class SshToolsApplicationClientPanel extends
	SshToolsApplicationPanel {
    private static final long serialVersionUID = 1990707760698961860L;
...
*/
	...
	return SshToolsConnectionPanel.showConnectionDialog(
		SshToolsApplicationClientPanel.this, profile,
...
*/
	...
	String prefsDir = super.getApplication()
		.getApplicationPreferencesDirectory().getAbsolutePath();
	...
	if (isConnected()) {
		...
		try {
		    SshToolsApplicationContainer c = (SshToolsApplicationContainer) application
			    .newContainer();
	...
	Runnable r = new Runnable() {
		...
		getStatusBar().setHost(getCurrentConnectionProfile().getHost(),
			getCurrentConnectionProfile().getPort());
		getStatusBar().setUser(
		...
		try {
		    ...
		    ssh.connect(
			    ...
			    (getHostKeyVerification() == null) ? new SinkHostKeyVerification()
				    : getHostKeyVerification());

		    // Set the remote id if we can find on
		    try {
			RemoteIdentification rid = RemoteIdentificationFactory
				.getInstance(ssh.getServerId(), ssh
					.getConnectionProperties().getHost());
			getStatusBar().setRemoteId(
				rid.getName(ssh.getServerId()));
	...
	Thread thread = new SshThread(r, application.getApplicationName()
		+ " connection", true);
	thread.start();
    }
	...
	java.util.List<String> supported = SshAuthenticationClientFactory
		.getSupportedMethods();
	...
	if (supported.contains("publickey")) {
		...
		try {
		    SshAgentClient agent = SshAgentClient
			    .connectLocalAgent("SSHTerm",
	...
	java.util.List<String> auths = null;
	...
	// First look to see if we have any authentication methods available
	// in the profile properties object as this will override a manual
	// selection
	...
	while (!completed
		...
		if (obj instanceof Map.Entry) {
		    ...
		    auth = SshAuthenticationClientFactory
			    .newInstance((String) obj);
		    auth.setUsername(getCurrentConnectionProfile()
			    .getUsername());
		} else {
		    closeConnection(true);
	    ...
	    if (auth != null) {
		// The password authentication client can act upon requests to
		// change the password

		/*
		 * if (auth instanceof PasswordAuthenticationClient) {
		 * PasswordAuthenticationDialog dialog = new
		 * PasswordAuthenticationDialog(SshTerminalPanel.this);
		 * ((PasswordAuthenticationClient)
		 * auth).setAuthenticationPrompt(dialog); (
		...
		if (result == AuthenticationProtocolState.COMPLETE) {
		    ...
		    // If successfull add to the connections list so we can save
		    // later
		    getCurrentConnectionProfile().addAuthenticationMethod(auth);
	...
	if (instance instanceof PasswordAuthenticationClient) {
	    ...
	    ((KBIAuthenticationClient) instance)
		    .setKBIRequestHandler(new KBIRequestHandlerDialog(
	...
	String verString = ConfigurationLoader.getVersionString(
	...
	public int compare(StandardAction o1, StandardAction o2) {
	    int i = ((Integer) o1.getValue(StandardAction.MENU_ITEM_GROUP))
		    .compareTo((Integer) o2
