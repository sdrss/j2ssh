...
public abstract class SshToolsApplicationPanel extends JPanel {
    ...
    /**  */
    protected Vector<StandardAction> actions = new Vector<StandardAction>();

    /**  */
    protected HashMap<String, Boolean> actionsVisible = new HashMap<String, Boolean>();

    /**  */
    protected boolean toolsVisible;

    /**  */
    protected Vector<ActionMenu> actionMenus = new Vector<ActionMenu>();
	...
	for (Iterator<StandardAction> i = actions.iterator(); i.hasNext();) {
	    StandardAction a = i.next();
	...
	for (Iterator<StandardAction> i = enabledActions.iterator(); i
		.hasNext();) {
	    StandardAction a = i.next();

	    if (Boolean.TRUE.equals((Boolean) a
		    .getValue(StandardAction.ON_TOOLBAR))) {
		v.addElement(a);
	    }
	}
	...
	for (Iterator<StandardAction> i = v.iterator(); i.hasNext();) {
	    StandardAction z = i.next();

	    if ((grp != null)
		    && !grp.equals((Integer) z
			    .getValue(StandardAction.TOOLBAR_GROUP))) {
		toolBar.add(new ToolBarSeparator());
	    }

	    if (Boolean.TRUE.equals((Boolean) z
		    .getValue(StandardAction.IS_TOGGLE_BUTTON))) {
	...
	for (Iterator<StandardAction> i = enabledActions.iterator(); i
	    ...
	    if (Boolean.TRUE.equals((Boolean) a
		    .getValue(StandardAction.ON_CONTEXT_MENU))) {
		c.addElement(a);
	    }
	}
	...
	for (Iterator<StandardAction> i = c.iterator(); i.hasNext();) {
	    StandardAction z = i.next();

	    if ((grp != null)
		    && !grp.equals((Integer) z
			    .getValue(StandardAction.CONTEXT_MENU_GROUP))) {
	...
	for (Enumeration<StandardAction> e = enabledActions.elements(); e
		.hasMoreElements();) {
	    StandardAction a = e.nextElement();

	    if (Boolean.TRUE.equals((Boolean) a
		    .getValue(StandardAction.ON_MENUBAR))) {
		v.addElement(a);
	    }
	}

	Vector<ActionMenu> menus = (Vector<ActionMenu>) actionMenus.clone();
	...
	for (Iterator<StandardAction> i = v.iterator(); i.hasNext();) {
		...
		log.error("Action " + z.getName()
			+ " doesnt specify a value for "
			+ StandardAction.MENU_NAME);
	    } else {
	...
	for (Iterator<ActionMenu> i = menus.iterator(); i.hasNext();) {
	    ActionMenu m = i.next();
	    Vector<StandardAction> x = (Vector<StandardAction>) map.get(m.name);
		...
		for (Iterator<StandardAction> j = x.iterator(); j.hasNext();) {
		    StandardAction a = j.next();
		    Integer g = (Integer) a
			    .getValue(StandardAction.MENU_ITEM_GROUP);
	...
	try {
		...
		Throwable cause = (Throwable) method.invoke(exception, new Object[]{null});
	...
	if (application != null) {
	    verString = ConfigurationLoader.getVersionString(
		    application.getApplicationName(),
		    application.getApplicationVersion());
	}

	if (container != null) {
	    container.setContainerTitle((file == null) ? verString : (verString
		    + " [" + file.getName() + "]"));
	}
    }
    ...
    public static class ActionMenu implements Comparable<ActionMenu> {
	int weight;
	int mnemonic;
	String name;
	String displayName;
	...
	public int compareTo(ActionMenu o) {
	    ...
	    return (i == 0) ? displayName
		    .compareTo(((ActionMenu) o).displayName) : i;
	}
    }

    class ToolBarActionComparator implements Comparator<StandardAction> {
	public int compare(StandardAction o1, StandardAction o2) {
	    int i = ((Integer) o1.getValue(StandardAction.TOOLBAR_GROUP))
		    .compareTo((Integer) o2
			    .getValue(StandardAction.TOOLBAR_GROUP));

	    return (i == 0) ? ((Integer) ((StandardAction) o1)
		    .getValue(StandardAction.TOOLBAR_WEIGHT))
		    .compareTo((Integer) ((StandardAction) o2)
			    .getValue(StandardAction.TOOLBAR_WEIGHT)) : i;
	}
    }

    class ContextActionComparator implements Comparator<StandardAction> {
	public int compare(StandardAction o1, StandardAction o2) {
	    int i = ((Integer) o1.getValue(StandardAction.CONTEXT_MENU_GROUP))
		    .compareTo((Integer) o2
			    .getValue(StandardAction.CONTEXT_MENU_GROUP));

	    return (i == 0) ? ((Integer) o1
		    .getValue(StandardAction.CONTEXT_MENU_WEIGHT))
		    .compareTo((Integer) o2
			    .getValue(StandardAction.CONTEXT_MENU_WEIGHT)) : i;
	}
    }

    class MenuItemActionComparator implements Comparator<StandardAction> {
	public int compare(StandardAction o1, StandardAction o2) {
	    int i = ((Integer) ((StandardAction) o1)
		    .getValue(StandardAction.MENU_ITEM_GROUP))
		    .compareTo((Integer) ((StandardAction) o2)
			    .getValue(StandardAction.MENU_ITEM_GROUP));

	    return (i == 0) ? ((Integer) ((StandardAction) o1)
		    .getValue(StandardAction.MENU_ITEM_WEIGHT))
		    .compareTo((Integer) ((StandardAction) o2)
			    .getValue(StandardAction.MENU_ITEM_WEIGHT)) : i;
	}
    }
