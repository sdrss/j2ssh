...
public class HostsTab extends JPanel implements OptionsTab, ActionListener {
    private static final long serialVersionUID = 5437928845356841080L;
    ...
    private JList<HostsListModel> hosts;
	...
	model = new HostsListModel();
	hosts = new JList<HostsListModel>(model);
	...
	IconWrapperPanel w = new IconWrapperPanel(
		new ResourceIcon(GLOBAL_ICON), s);
	...
	/*
	 * else if (evt.getSource() == deny) { int i = hosts.getSelectedIndex();
	 * HostWrapper w = model.getHostAt(i); w.allow = false;
	...
	HostWrapper w = ((model.getSize() > 0) && (hosts.getSelectedValues().length == 1)) ? model
		.getHostAt(hosts.getSelectedIndex()) : null;
	...
	try {
	    Map<String, Map<String, SshPublicKey>> map = hostKeyVerifier
		    .allowedHosts();
	     ...
	     * log.debug("Checking if any denied hosts need to be removed"); for
	     * (int i = list.size() - 1; i >= 0; i--) { String h = (String)
	     * list.get(i); if (log.isDebugEnabled()) {
	     * log.debug("Looking for host " + h); } HostWrapper w =
	     * model.getHost(h); if (w == null) { if (log.isDebugEnabled()) {
...
*/
	...
	String host;
	SshPublicKey key;
	Map<String, SshPublicKey> keys;

	HostWrapper(boolean allow, String host, Map<String, SshPublicKey> keys) {
	    this.allow = allow;
	    this.host = host;
	    this.keys = keys;
	}
	...
	private static final long serialVersionUID = 1L;
	java.util.List<HostWrapper> hosts;

	public HostsListModel() {
	    hosts = new java.util.ArrayList<HostWrapper>();
	    refresh();
	}
	...
	public void refresh() {
	    ...
	    final Map<String, Map<String, SshPublicKey>> map = hostKeyVerifier
		    .allowedHosts();

	    for (Iterator<String> i = map.keySet().iterator(); i.hasNext();) {
		final String k = i.next();
		final Map<String, SshPublicKey> keys = map.get(k);
		hosts.add(new HostWrapper(true, k, keys));
	    }

	    /*
	     * java.util.List list = hostKeyVerifier.deniedHosts(); for
	     * (Iterator i = list.iterator(); i.hasNext(); ) { String h =
	     * (String) i.next(); hosts.add(new HostWrapper(false, h, null)); }
	     */
	    fireContentsChanged(this, 0, getSize() - 1);
	}

	public HostWrapper getHost(String name) {
	    for (Iterator<HostWrapper> i = hosts.iterator(); i.hasNext();) {
		HostWrapper w = i.next();
