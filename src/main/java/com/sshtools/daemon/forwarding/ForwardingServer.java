...
public class ForwardingServer implements ChannelFactory, GlobalRequestHandler {
    ...
    private List<String> channelTypes = new Vector<String>();
    private List<ForwardingConfiguration> localForwardings = new Vector<ForwardingConfiguration>();
    private List<ForwardingListener> remoteForwardings = new Vector<ForwardingListener>();
	...
	connection.addChannelFactory(
		ForwardingSocketChannel.LOCAL_FORWARDING_CHANNEL, this);
	connection.allowGlobalRequest(ForwardingClient.REMOTE_FORWARD_REQUEST,
		this);
	connection.allowGlobalRequest(
		ForwardingClient.REMOTE_FORWARD_CANCEL_REQUEST, this);
    }

    /*
     * public List getChannelType() { return channelTypes; }
     */
    public Channel createChannel(String channelType, byte[] requestData)
	    throws InvalidChannelException {
	if (!channelType
		.equals(ForwardingSocketChannel.LOCAL_FORWARDING_CHANNEL)) {
	...
	try {
	    ...
	    if (requestName
		    .equals(ForwardingClient.REMOTE_FORWARD_CANCEL_REQUEST)) {
    ...
    protected ForwardingConfiguration getLocalForwardingByAddress(
	    ...
	    throws ForwardingConfigurationException {
	try {
	    Iterator<ForwardingConfiguration> it = localForwardings.iterator();
    ...
    protected ForwardingConfiguration getRemoteForwardingByAddress(
	    ...
	    throws ForwardingConfigurationException {
	Iterator<ForwardingListener> it = remoteForwardings.iterator();
	ForwardingConfiguration config;
	...
	try {
	    ForwardingListener listener = new ServerForwardingListener(
	...
	public ForwardingSocketChannel createChannel(String hostToConnect,
		...
		ForwardingSocketChannel channel = createForwardingSocketChannel(
			 ...
			 * .getHostAddress(), ( (InetSocketAddress)
