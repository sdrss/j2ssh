...
public abstract class SshServer {
	...
	try {
	    commandServerSocket = new ServerSocket(
		    ((ServerConfiguration) ConfigurationLoader.getConfiguration(ServerConfiguration.class))
			    .getCommandPort(), 50, InetAddress.getLocalHost());
	...
	listener = new ConnectionListener(
		((ServerConfiguration) ConfigurationLoader
			...
			.getListenAddress(),
		((ServerConfiguration) ConfigurationLoader
			.getConfiguration(ServerConfiguration.class)).getPort());
	listener.start();
    }
	...
	// Allow the Connection Protocol to be accepted by the Authentication
	// Protocol
	...
	public void run() {
		...
		for (Iterator<TransportProtocolServer> it = activeConnections
			.iterator(); it.hasNext();) {
		    it.next().disconnect("The server is shuting down");
		}
	...
	public void stop() {
		...
		log.warn(
			"The listening socket reported an error during shutdown",
			ioe);
	    }
	}
    }
