...
public class SessionChannelServer extends IOChannel {
    ...
    private static Map<String, AllowedSubsystem> allowedSubsystems = new HashMap<String, AllowedSubsystem>();
	...
	if (requestType.equals("auth-agent-req")) {
		...
		agent = SshAgentForwardingListener.getInstance(
			thread.getSessionIdString(), connection);
	...
	public void run() {
		...
		if (exitcode == 9999999) {
			...
			connection.sendChannelRequest(
				SessionChannelServer.this, "exit-status",
				false, baw.toByteArray());
		    }
