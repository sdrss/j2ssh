...
public class PlatformConfiguration extends DefaultHandler {
    ...
    private Map<String, String> nativeSettings = new HashMap<String, String>();
    ...
    private Map<String, VFSMount> vfsMounts = new HashMap<String, VFSMount>();
     ...
     * @return
     */
    public Map<String, VFSMount> getVFSMounts() {
	return vfsMounts;
    }
	...
	Iterator<Map.Entry<String, String>> it = nativeSettings.entrySet().iterator();
	...
	Iterator<Map.Entry<String, VFSMount>> it2 = vfsMounts.entrySet().iterator();

	String path;
	String mount;

	while (it2.hasNext()) {
	    Map.Entry<String, VFSMount> entry = it2.next();
	    ...
	    xml += ("   "
		    ...
		    + VFSMOUNT_ELEMENT
		    + " "
		    + (mount.equals(path) ? "" : (MOUNT_ATTRIBUTE + "=\""
			    + entry.getKey().toString() + "\" "))
		    + PATH_ATTRIBUTE + "=\"" + entry.getValue().toString() + "\"/>\n");
	}
	...
	public void startElement(String uri, String localName, String qname,
		...
		if (qname.equals(VFSMOUNT_ELEMENT)) {
			...
			if (!mount.trim().equals("/")) {
				...
				if (permissions == null) {
				    currentMount
					    .setPermissions(new VFSPermission(
						    "default"));
				} else {
				    currentMount
					    .setPermissions(new VFSPermission(
				...
				if (!vfsMounts.containsKey(currentMount
					.getMount())) {
