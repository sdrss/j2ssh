...
public class ServerConfiguration extends DefaultHandler {
    ...
    private Map<String, AllowedSubsystem> allowedSubsystems = new HashMap<String, AllowedSubsystem>();
	...
	if (currentElement == null) {
		...
		if (qname.equals("ServerHostKey")) {
		    ...
		    log.debug("Subsystem Type=" + type + " Name=" + name
			    + " Provider=" + provider);
		    allowedSubsystems.put(name, new AllowedSubsystem(type,
			    name, provider));
	...
	if (currentElement != null) {
		...
		try {
		    sessionChannelImpl = ConfigurationLoader
			    .getExtensionClass(value);
		} catch (Exception e) {
    ...
    /*
     * public Class getSessionChannelImpl() { return sessionChannelImpl; }
     */
     ...
     * @return
     */
    public Map<String, AllowedSubsystem> getSubsystems() {
	...
	Iterator<Map.Entry<String, SshPrivateKey>> it = serverHostKeys
		.entrySet().iterator();

	while (it.hasNext()) {
	    Map.Entry<String, SshPrivateKey> entry = it.next();
	    xml += ("   <ServerHostKey PrivateKeyFile=\"" + entry.getValue() + "\"/>\n");
	}

	xml += "   <!-- Add any number of subsystem elements here -->\n";

	Iterator<Map.Entry<String, AllowedSubsystem>> it2 = allowedSubsystems
		.entrySet().iterator();

	while (it2.hasNext()) {
	    AllowedSubsystem subsystem = (AllowedSubsystem) it2.next()
		    .getValue();
	...
	Iterator<String> it3 = allowedAuthentications.iterator();
	
	while (it3.hasNext()) {
	    xml += ("   <AllowedAuthentication>" + it3.next().toString() + "</AllowedAuthentication>\n");
	}

	xml += "   <!-- Specify any number of required authentications -->\n";
	
	Iterator<String> it4 = requiredAuthentications.iterator();

	while (it4.hasNext()) {
	    xml += ("   <RequiredAuthentication>" + it4.next().toString() + "</RequiredAuthentication>\n");
	}
